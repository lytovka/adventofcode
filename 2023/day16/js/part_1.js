...
const getEnergizedTiles = (contraption, initPos) => {
	...
	for (const [id, value] of beamsMap){
		...
		if(bRow < 0 || bRow > contraption.length - 1 || bCol < 0 || bCol > contraption[0].length - 1) {
			beamsMap.delete(id);
			continue;
		}

		if(energizedTiles.has(`${bRow},${bCol}`) && energizedTiles.get(`${bRow},${bCol}`).includes(value)){
			beamsMap.delete(id)
			continue;
		}
		...
		if(dRow === 0 && dCol === 0){
			// hardcoded
			if(tile === "\\"){
				beamsMap.set(crypto.randomUUID(), [[bRow+1, bCol], [1, 0]].toString())	
				energizedTiles.set([bRow,bCol].toString(), value)
				beamsMap.delete(id)
				continue;
			}
			beamsMap.set(crypto.randomUUID(), [[bRow, bCol + 1], [0, 1]].toString())		
			energizedTiles.set([bRow,bCol].toString(), value)
			beamsMap.delete(id)
			continue;
		}
		if(tile === "."){
			if(dRow === 1 || dRow === - 1){
				beamsMap.set(crypto.randomUUID(), [[bRow + dRow, bCol], [dRow, dCol]].toString())
				energizedTiles.set([bRow,bCol].toString(), value)	
				beamsMap.delete(id)
				continue;
			}
			if(dCol === 1 || dCol === -1){
				beamsMap.set(crypto.randomUUID(), [[bRow, bCol + dCol], [dRow, dCol]].toString())
				energizedTiles.set([bRow,bCol].toString(), value)	
				beamsMap.delete(id)
				continue;
			}
...
console.log(getEnergizedTiles(rows, [[0,0], [1,0]]).size)
