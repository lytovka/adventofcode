...
program.name("aoc").description("CLI to interact with AdventOfCode");
program
  .command("input")
  .arguments("<year> <day>")
  .action((year, day) => {
...
program
  ...
  .arguments("<year> <day> <part>")
  .action(async (year, day, part) => {
    commands.setupJsFile(year, day, part).then(console.log, console.error);
  });
program
  .command("submit")
  ...
  .action(async (year, day, part, answer) => {
    commands
      .submitPuzzle(year, day, part, answer)
