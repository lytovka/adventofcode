...
for (const pattern of patterns) {
  ...
  const best =
    statsVertical.bestScore >= statsHorizontal.bestScore
      ? statsVertical
      : statsHorizontal;
  console.log(best);
  console.log("-------");
...
console.log(
  bestScores
    .filter((val) => val.colsLeft)
    .reduce((acc, val) => acc + val.colsLeft, 0) +
    100 *
      bestScores
        .filter((val) => val.rowsAbove)
        .reduce((acc, val) => acc + val.rowsAbove, 0),
);
